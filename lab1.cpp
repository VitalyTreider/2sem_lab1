...
int main()
{
    ...
    {
        Rect rect; // инициализируется мусором, случайными значениями
                   // т.к. мы явно не задали конструктор, то вызывается конструктор по умолчанию
                   // который по сути ничего не делает, он оставляет мусор который хранился в памяти
    ...
    {
        Rect rect(1, 2, 3, 4); // программа не смогла собраться из-за предыдущего кода, где мы не передаем никакие параметры
                               // чтобы этого избежать, нужно создать конструктор по умолчанию, который проинициализирует все нулями
    ...
    {
        Rect rect1(1, 2, 3, 4); // вызывается явный конструктор, который я написал
        Rect rect2 = rect1; // неявный конструктор копирования, выполняет побитовое копирование всех полей
        Rect rect3(rect1); // неявный конструктор копирования (другой синтаксис)
...
// Деструктор выполняет освобождение использованных объектом ресурсов и удаление 
// нестатических переменных объекта. Деструктор автоматически вызывается, когда удаляется объект.
// В моем классе он задан неявно. В моем случае деструктор удаляет сам объект и все его переменные (когда завершается блок)
...
//     4) динамически созданные объекты удаляются при применении к указателю на объект оператора delete
...

    {
        ...
        Rect *pR = new Rect(1,2,1,2);   // явный конструктор
        {
            Rect r2(r1);    // конструктор копирования
            Rect arRect[2];    // т.к. создан массив, вызвалось 2 конструктора по умолчанию
            for(int i = 0; i < 3; i++)
            {
                static Rect r3 (i,i,i,i) ; // 1цикл-явный констр. 2цикл - ничего. 3цикл - ничего, деструктор только после завершения main, т.к. static
                Rect r4(*pR); // 1цикл - констр копир, дестр 2цикл -  констр копи, дестр. 3цикл - тож самое
                Rect r5(i,i,i,i); // 1цикл - констр явно, дестр 2цикл - явн констр, дестр. 3цикл - тож самое
            }
        } // деструктор для r2, arRect[2] 
    ...
    //{
    //    Rect r1;
    //    std::cout << r1.c1 << std::endl; // c1 - в приватном поле, следовательно можем обращаться только внутри класса
    //}
    ...
    {
       Rect r;
       std::cout << r.get_left() << "; " << r.get_right() << '\n'; // вывел 0 0, как и должен
       r.set_all(1, 2, 3, 4);
       std::cout << r.get_top() << "; " << r.get_bottom() << '\n'; // вывел 3 4, как и должен всё работает корректно
    } 
...
// функция-член - это функция которая присвоена определенному классу. Есть скрытый параметр this, вызывается obj.func().
